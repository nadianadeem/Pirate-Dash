...
public class gameMaster : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        shots.text = "SHOTS " + player.Shots;

        //The time is rounded to seconds, as a decimal it is unneccessary for the reader.
        timer.text = "TIMER " + (Mathf.RoundToInt(Time.timeSinceLevelLoad));
       ...
       if (player.Lives <= 0)
        {
            Destroy(gameObject);
            scoreBonus = Mathf.RoundToInt(Time.timeSinceLevelLoad);
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.tag == "shark")
        {
            //Removes sprite from the scene.
            Destroy(collision.gameObject);
            player.Health = player.Health - shark.Damage;
        }
    }
